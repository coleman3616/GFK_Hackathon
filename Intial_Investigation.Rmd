...
UPC_Hack_Cereal2 %>%
  ...
  nrow()
```
* UPC Values are all the same for every store and Every UPC value is shown once
per store


```{r }

upc_select = 1600015087

 
  

full_UPC <- rbind(UPC_Hack_Cereal2,UPC_Hack_Cleaners2)
...
full_UPC %>%
  ...
  ggplot(aes(y = price, x = date, color = Store)) +
  geom_point()
