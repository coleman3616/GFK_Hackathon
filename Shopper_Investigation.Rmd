...
get_weighted <- function(.data, weight) {
  ...
  for (i in 3:nrow(.data)) {
    weighted_data[i,1] <- .data[i,1]
    for (j in 2:ncol(.data)) {
      weighted_data[1, j] <- .data[1,j]
      weighted_data[i, j] <- as.numeric(weighted_data[i, j]) * as.numeric(weighted_data[weight, j])
    }
  }
  weighted_data <- weighted_data[-c(1,2),]
...
s1_weighted_data <- get_weighted(s1_mobile_devices, 2)
s1_unweighted_data <- get_weighted(s1_mobile_devices, 1)

view(s1_weighted_data)
view(s1_unweighted_data)

ggplot(s1_weighted_data, aes(Total)) + 
  geom_bar() 
